...
if(isset($_POST['cadastrar'])){
  ...
  if(!empty(trim($_POST['titulo']))){ 
      ...
      $idciclo=mysqli_escape_string($conexao,$_POST['idciclo']); 
        ...
        $existe=mysqli_num_rows(mysqli_query($conexao, "select idclasse from classes where titulo='$titulo'"));
      
          if($existe==0){

                $salvar= mysqli_query($conexao,"INSERT INTO `classes` (titulo, idciclo) VALUES ('$titulo', '$idciclo')");
              ...
              $erros[]="Ocorreu um erro Ao Cadastrar a classe";

            } 
          }else{

        $erros[]="Já existe um classe com esse título";
      }
...
include("cabecalho.php") ; ?>
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-2 text-gray-800">Lista de classes no ano lectivo (<?php echo $anolectivo_escolhido; ?>)</h1>
          <p class="mb-4">A seguir vai a lista de classes disponíveis na instituição</p>
    ...
    <button id="myBtn" class="btn btn-success">  <i class="fas fa-fw fa-plus"></i> Acrescentar uma nova classe</button>
    ...
    <div id="myModal" class="modal"  >
        <div class="modal-content">
          ...
          <form class="user" method="post" action=""> 
                <h3>Cadastrando uma nova classe</h3> <br>
                    <div class="form-group">
                          <?php

                                $lista=mysqli_query($conexao, "select * from classes"); 
                        
                          ?>
                      <input type="text" name="titulo" autocomplete="off" list="datalist2" class="form-control"  placeholder="Título para a classe" required="">
                    ...
                    <br>
                       <input type="submit" name="cadastrar" value="Dar abertura a nova classe" class="btn btn-success" style="float: rigth;">
                ...
                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                        ...
                        $lista=mysqli_query($conexao, "select * from classes"); 
                         while($exibir = $lista->fetch_array()){
                           ...
                           $idclasse=$exibir["idclasse"];
                            ...
                            $periodos=mysqli_num_rows(mysqli_query($conexao,"SELECT distinct(idperiodo) from turmas where idclasse='$idclasse' and idanolectivo='$idanolectivo'"));

                            $cursos=mysqli_num_rows(mysqli_query($conexao,"SELECT distinct(idcurso) from turmas where idclasse='$idclasse' and idanolectivo='$idanolectivo'"));

                            $turmas=mysqli_num_rows(mysqli_query($conexao,"SELECT distinct(idturma) from turmas where idclasse='$idclasse' and idanolectivo='$idanolectivo'"));

                            $alunos=mysqli_num_rows(mysqli_query($conexao,"SELECT distinct(idaluno) from matriculaseconfirmacoes, turmas where matriculaseconfirmacoes.idturma=turmas.idturma and turmas.idclasse='$idclasse' and matriculaseconfirmacoes.idanolectivo='$idanolectivo' and matriculaseconfirmacoes.estatus='activo'"));
                  ...
                  ?>
                    <tr>  
                      ...
                      <td align="center" title="Veja mais opções sobre esse classe">
                         <a  href="classe.php?idclasse=<?php echo $exibir["idclasse"]; ?>" <?php echo $exibir["idclasse"]; ?>><i  class="fas fa-eye" ></i> </a>
                      </td>
                    </tr> 
                    <?php } ?> 
                  </tbody>
