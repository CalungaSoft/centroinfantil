...
if(isset($_POST['cadastrar'])){
  ...
  if(!empty(trim($_GET['idaluno']))){ 
      ...
      $tipodematricula=mysqli_escape_string($conexao, trim($_POST['tipodematricula']));
                   ...
                   $existe=mysqli_num_rows(mysqli_query($conexao, "select idaluno from matriculaseconfirmacoes where idaluno='$idaluno' and idturma='$idturma'"));
                         ...
                         if($existe==0){
                          ...
                          if($salvar){
                                ...
                                $salvar_financas=mysqli_query($conexao,"INSERT INTO `entradas` (`identrada`, `idfuncionario`, `descricao`, `tipo`, `idtipo`, `valor`, `divida`, `idaluno`, `idturma`, `datadaentrada`, formadepagamento, idanolectivo) VALUES (NULL, '$idlogado', '$descricao', '$tipodematricula', '$idmatriculaeconfirmacao', '$valorpago', '$divida', '$idaluno', '$idturma', STR_TO_DATE('$datadamatricula', '%d/%m/%Y'), '$formadepagamento', '$idanolectivo')");


                                      $identrada=mysqli_fetch_array(mysqli_query($conexao, "SELECT identrada from  entradas where idaluno='$idaluno' and tipo='$tipodematricula' order by identrada desc limit 1"))[0]; 
                          

                                          if($salvar_financas){  

                                            $acerto[]="$tipodematricula Feita com Sucesso <br> <a class='btn btn-info' href='aluno.php?idaluno=".$idaluno."'> Click aqui para ver mais dados sobre esse aluno</a> | <a class='btn btn-success' href='pdf/recibopagamento.php?identrada=".$identrada."'> Imprimir Recibo </a>"; 

                                          }else{

                                            $erros[]="Ocorreu um erro ao fazer a $tipodematricula do aluno | No Registro de financas";
                                     ...
                                     $erros[]="Ocorreu um erro ao fazer a $tipodematricula do aluno";
                                }
                        }else{

                                     $erros[]="Já existe uma $tipodematricula desse aluno nessa turma";
                                }
...
include("cabecalho.php"); ?>
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-4 text-gray-800">Rematrícula do Aluno</h1>
          <?php 
            if(!empty($erros)):
          ...
          <div class="row">
            ...
            <div class="col-lg-7">
              ...
              <div class="card shadow mb-4">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary">Rematrícula</h6>
                </div>
                <div class="card-body">
            ...
            <script>
             ...
             turma.addEventListener("change", function(){
                    ...
                    var idturma=this.value;
                      ...
                      if(idturma=='0'){
                         ...
                         $.ajax({
                          url:"cadastro/pesquisarturmarematricula.php",
                          method:"POST",
                          data:{idturma},
             ...
             anolectivo.addEventListener("change", function(){
                    ...
                    var idanolectivo=this.value;
                     $.ajax({
                          url:"cadastro/pesquisaranolectivorematricula.php",
                          method:"POST",
                          data:{idanolectivo},
                          success:function(data){
