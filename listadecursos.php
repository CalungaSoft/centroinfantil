...
include("cabecalho.php") ; ?>
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-2 text-gray-800">Lista de cursos no ano lectivo (<?php echo $anolectivo_escolhido; ?>)</h1>
          <p class="mb-4">A seguir vai a lista de cursos disponíveis na instituição</p>
 ...
 <button id="myBtn" class="btn btn-success">  <i class="fas fa-fw fa-plus"></i> Acrescentar um novo curso</button>
    ...
    <div id="myModal" class="modal"  >
        <div class="modal-content">
          ...
          <form class="user" method="post" action=""> 
                <h3>Cadastrando um novo curso</h3> <br>
                    <div class="form-group">
                          <?php

                                $lista=mysqli_query($conexao, "select * from cursos"); 
                        
                          ?>
                      <input type="text" name="titulo" autocomplete="off" list="datalist2" class="form-control"  placeholder="Título para o curso" required="">
                    ...
                    <br>
                       <input type="submit" name="cadastrar" value="Dar abertura ao novo curso" class="btn btn-success" style="float: rigth;">
                        ...
                        $lista=mysqli_query($conexao, "select * from cursos"); 
                         while($exibir = $lista->fetch_array()){
                           ...
                           $idcurso=$exibir["idcurso"];
                            ...
                            $periodos=mysqli_num_rows(mysqli_query($conexao,"SELECT distinct(idperiodo) from turmas where idcurso='$idcurso' and idanolectivo='$idanolectivo'"));

                            $classes=mysqli_num_rows(mysqli_query($conexao,"SELECT distinct(idclasse) from turmas where idcurso='$idcurso' and idanolectivo='$idanolectivo'"));

                            $turmas=mysqli_num_rows(mysqli_query($conexao,"SELECT distinct(idturma) from turmas where idcurso='$idcurso' and idanolectivo='$idanolectivo'"));

                            $alunos=mysqli_num_rows(mysqli_query($conexao,"SELECT distinct(idaluno) from matriculaseconfirmacoes, turmas where matriculaseconfirmacoes.idturma=turmas.idturma and turmas.idcurso='$idcurso' and matriculaseconfirmacoes.idanolectivo='$idanolectivo' and matriculaseconfirmacoes.estatus='activo'"));

                  ?>
                    <tr>  
                      <td> <a  href="curso.php?idcurso=<?php echo $exibir["idcurso"]; ?>"> <?php echo $exibir['titulo']; ?> </a></td> 
                      ...
                      <td align="center" title="Veja mais opções sobre esse curso">
                         <a  href="curso.php?idcurso=<?php echo $exibir["idcurso"]; ?>" <?php echo $exibir["idcurso"]; ?>><i  class="fas fa-eye" ></i> </a>
                      </td>
                    </tr> 
                    <?php } ?> 
                  </tbody>
