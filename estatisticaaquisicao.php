...
if(!isset($_GET['anodevenda'])){  
    ...
    for ($i=1; $i <=12 ; $i++) { 
        $dia[$i]=mysqli_num_rows( mysqli_query($conexao,"SELECT idmatriculaeconfirmacao from matriculaseconfirmacoes where MONTH(data)='$i'"));
      } 
          $totaldematriculaseconfirmacoes = mysqli_num_rows(mysqli_query($conexao,"SELECT idmatriculaeconfirmacao from matriculaseconfirmacoes"));
  
   
  }else if(isset($_GET['anodevenda'])){ 
  
      for ($i=1; $i <=12 ; $i++) { 
      $dia[$i]=mysqli_num_rows( mysqli_query($conexao,"SELECT idmatriculaeconfirmacao from matriculaseconfirmacoes where '$anodevenda'=YEAR(data)  and  MONTH(data)='$i' "));
    }
      $totaldematriculaseconfirmacoes = mysqli_num_rows(mysqli_query($conexao,"SELECT idmatriculaeconfirmacao from matriculaseconfirmacoes where  '$anodevenda'=YEAR(data)"));
   ...
   include("cabecalho.php")?>
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-2 text-gray-800">Aquisição de alunos<?php if(isset($_GET['anodevenda'])){ echo "| $anodevenda"; }?> <?php if(isset($_GET['tipomarcado'])){ echo "($tipomarcado)"; }?>   </h1>
...
if(!isset($_GET['anodevenda'])){  
  ...
  $esseano=mysqli_num_rows(mysqli_query($conexao, "select idmatriculaeconfirmacao FROM matriculaseconfirmacoes")); 
  $outrosanos=0; 
}else{

  $esseano=mysqli_num_rows(mysqli_query($conexao, "select idmatriculaeconfirmacao FROM matriculaseconfirmacoes where YEAR(data)='$anodevenda'")); 
  $outrosanos=mysqli_num_rows(mysqli_query($conexao, "select idmatriculaeconfirmacao FROM matriculaseconfirmacoes where YEAR(data)!='$anodevenda'")); 
}
...
<!-- Content Row -->
            ...
            <div class="col-xl-4 col-lg-5">
              <div class="card shadow mb-4">
                <!-- Card Header - Dropdown -->
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-primary">Representação da Aquisição de alunos esse ano em relação aos outros</h6>
                  <div class="dropdown no-arrow">
                ...
                <div class="card-body">
                  ...
                  <div class="mt-4 text-center small">
                    ...
                    <span class="mr-2">
                      <i class="fas fa-circle text-gray"></i> alunos Adquiridos Outros Anos
                    </span> 
                  </div>
                </div>
              </div>
...
var myPieChart = new Chart(ctx, {
  type: 'doughnut',
  data: {
    labels: ["alunos Outros Anos", "alunos deste ano"],
    datasets: [{
