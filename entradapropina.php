...
$identrada=mysqli_fetch_array(mysqli_query($conexao," SELECT identrada FROM entradas where tipo='Propina' and idtipo='$idpropina'"))[0];
 }
                ...
                $salvar_financas=mysqli_query($conexao,"INSERT INTO `entradas` (`identrada`, `idfuncionario`, `descricao`, `tipo`, `idtipo`, `valor`, `divida`, `idaluno`, `idturma`, `datadaentrada`, formadepagamento, idanolectivo) VALUES (NULL, '$idlogado', '$descricao', '$tipo','$idtipo', '$valor', '$nova_divida', '$idaluno', '$idturma', CURRENT_TIMESTAMP, '$formadepagamento', '$idanolectivo')");
                      ...
                      if($salvar_financas){
                        ...
                        $dados_do_pagamento=mysqli_fetch_array(mysqli_query($conexao," SELECT  sum(valor) as totalpago FROM entradas where tipo='$tipo' and idtipo='$idtipo'"));
                          ...
                          $actualizandopagamentos=mysqli_query($conexao,"UPDATE `propinas` SET `valorpago` = '$totalpago' WHERE `propinas`.`idpropina` = '$idtipo'");
...
if(isset($_POST['editaralteracoes'])){
    ...
    $datadaentrada=mysqli_escape_string($conexao, trim($_POST['datadaentrada']) );  
                ...
                $novo="($descricao) | Pago: $valor  | F. Pag: $formadepagamento | $datadaentrada <a href=entradapropina.php?identrada=$identrada>Clique para ver</a>";
            ...
            if($guardar2){
               ...
               $actualizando=mysqli_query($conexao, "UPDATE `entradas` SET `datadaentrada` = '$datadaentrada', `descricao` = '$descricao', `valor` = '$valor', `formadepagamento` = '$formadepagamento' WHERE identrada = '$identrada'");
                  ...
                  $zerando_dividas=mysqli_query($conexao,"UPDATE `entradas` SET  `divida` ='0' WHERE tipo='$tipo' and idtipo='$idtipo'");
                   ...
                   $preco=mysqli_fetch_array(mysqli_query($conexao," SELECT sum(preco-desconto+multa) as preco FROM propinas where idpropina='$idtipo' "))['preco'];
                   ...
                   $actualizando_nasmatriculas=mysqli_query($conexao,"UPDATE `propinas` SET  `valorpago` ='$valorpago' WHERE   idpropina='$idtipo' limit 1");
          ...
          if($nova_divida<0){
              $nova_divida=0;
                          ...
                          if($insirindo_dividas){
                                ...
                                $actualizandopagamentos=mysqli_query($conexao,"UPDATE `propinas` SET `preco` = '$preco', `desconto` = '$desconto', `multa` = '$multa' WHERE `propinas`.`idpropina` = '$idtipo'");
...
<?php
  ...
  <button id="myBtnreclamacoes" class="btn btn-info" title="Registrar Um Pagamento na data de hoje"><i  class="fas fa-money"></i>  Fazer um pagamento Novo</button>  
    ...
    <a class='btn btn-success' href='pdf/recibodepropina.php?idpropina=<?php echo "$dados_da_entrada[idtipo]";?>'> imprimir recibo Só de Propinas</a>
                                ...
                                <div class="row no-gutters align-items-center">
                                    ...
                                    <div class="row no-gutters align-items-center">
                                        ...
                                        <p id="mostra1">  <br> <?php
                                                 ...
                                                 $propina_paga=mysqli_fetch_array(mysqli_query($conexao,"SELECT YEAR(mespago) as ano, MONTH(mespago) as mes, propinas.* from propinas where idpropina='$idtipo'"));
            ...
            <script>
                                                            ...
                                                            $(document).on("click", ".deletevenda", function(event){
                                                                ...
                                                                if(confirm("Tens certeza que queres eliminar esse registro? Serão eliminados todos os dados financeiros relacionados com esse registro!")){
                                                                    ...
                                                                    $.ajax({
                                                                    url:'cadastro/deletevendapropina.php',
                                                                    method:'POST',
                                                                    data:{
                                                                        id:id
                                                                    },
                     ...
                     $(document).on("click", ".delete", function(event){
                                ...
                                if(confirm("Tens certeza que queres eliminar esse registro? Isso não apagará a dívida, pelo contrário só a acrescentará, isso em detrimento do valor pago apagado!")){
                                    $(this).closest('tr').remove(); 
                                    $.ajax({
                                    url:'cadastro/deleteentradapropina.php',
                                    method:'POST',
                                    data:{
                                        id
                                    },
