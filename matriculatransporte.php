...
if(isset($_POST['cadastrar'])){
  ...
  if(!empty(trim($idaluno))){ 
      ...
      $idtransporte=mysqli_escape_string($conexao, trim($_POST['idtransporte'])); 
      ...
      $formadepagamento=mysqli_escape_string($conexao, trim($_POST['formadepagamento']));
       ...
       $tipodealuno=mysqli_escape_string($conexao, trim($_POST['tipodealuno']));
                  ...
                  $dadoslectivos= mysqli_fetch_array(mysqli_query($conexao, "select * from transportes where idtransporte='$idtransporte' limit 1")); 
                   ...
                   $existe=mysqli_num_rows(mysqli_query($conexao, "select idaluno from matriculatransporte where idaluno='$idaluno' and idtransporte='$idtransporte'"));
                         ...
                         if($existe==0){
                           ...
                           $salvar=mysqli_query($conexao,"INSERT INTO `matriculatransporte` (`idmatriculatransporte`, `idaluno`, `idanolectivo`, `idtransporte`,   `preco`, `desconto`, `valorpago`, `transporte`, data,obs, tipodealuno, descontoparapropinas) VALUES (NULL, '$idaluno', '$idanolectivo', '$idtransporte',   '$preco', '$desconto', '$valorpago', '$transporte',   STR_TO_DATE('$datadamatricula', '%d/%m/%Y'), '$obamatricula', '$tipodealuno', '$descontoparapropinas')");
                          
                     $datadehoje=Date("Y-m-d");

                                           $salvar=mysqli_query($conexao,"UPDATE `matriculatransporte` SET data='$datadehoje' WHERE data='0000-00-00'");
                          ...
                          if($salvar){
                                 ...
                                 $idmatriculatransporte=mysqli_fetch_array(mysqli_query($conexao, "select idmatriculatransporte from matriculatransporte where idaluno='$idaluno' order by idmatriculatransporte desc limit 1"))[0];
                      
                                 $descricao="Registro de Matrículo no transporte";
                                $salvar_financas=mysqli_query($conexao,"INSERT INTO `entradas` (`identrada`, `idfuncionario`, `descricao`, `tipo`, `idtipo`, `valor`, `divida`, `idaluno`, `idturma`, `datadaentrada`, formadepagamento, idanolectivo) VALUES (NULL, '$idlogado', '$descricao', 'Matrícula transporte', '$idmatriculatransporte', '$valorpago', '$divida', '$idaluno', '0', STR_TO_DATE('$datadamatricula', '%d/%m/%Y'), '$formadepagamento', '$idanolectivo')");
                                     ...
                                     $erros[]="Já existe uma Matrícula desse aluno nesse transporte";
                                }
...
include("cabecalho.php"); ?>
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-4 text-gray-800">Matricula aluno no Transporte ( <a href="aluno.php?idaluno=<?php echo $idaluno; ?>"> <?php echo $Dados_do_aluno['nomecompleto']; ?> </a>)</h1>
...
?>
                ...
                <span id="transportedinamicas">
                    <span>transporte</span>
                    <div class="form-group">
                    <select name="transporte" id='escolhetransporte' required  class="form-control">
                        <option value="0">Escolha o transporte</option> 
                      <?php
                           $lista=mysqli_query($conexao,"SELECT * from transportes where idanolectivo='$anolectivo' order by titulo desc");
                          while($exibir = $lista->fetch_array()){ ?>
                          <option value="<?php echo $exibir["idtransporte"]; ?>"><?php echo $exibir["titulo"]; ?></option>
                        <?php } ?> 
                    </select> 
                    </div>
                </span>
            ...
            <script>
             ...
             anolectivo.addEventListener("change", function(){
                    ...
                    var idanolectivo=this.value;
                     $.ajax({
                          url:"cadastro/pesquisaranolectivotransporte.php",
                          method:"POST",
                          data:{idanolectivo},
                          success:function(data){

                          $("#transportedinamicas").html(data)
                          var dadoslectivos=document.getElementById('dadoslectivos');
                            dadoslectivos.innerHTML='';
