...
if(isset($_POST['cadastrar'])){
  ...
  if(!empty(trim($idaluno))){ 
      ...
      $datadareconfirmacao=mysqli_escape_string($conexao, trim($_POST['datadareconfirmacao'])); 
      $idturma=mysqli_escape_string($conexao, trim($_POST['idturma'])); 
      $obsreconfirmacao=mysqli_escape_string($conexao, trim($_POST['obsreconfirmacao']));  
      ...
      $tipodealuno=mysqli_escape_string($conexao, trim($_POST['tipodealuno']));
           $descontoparapropinas=mysqli_escape_string($conexao, trim($_POST['descontoparapropinas']));
                  ...
                  $dadoslectivos= mysqli_fetch_array(mysqli_query($conexao, "select * from turmas where idturma='$idturma' limit 1")); 
                           ...
                           $preco=mysqli_escape_string($conexao, trim($_POST['reconfirmacao']));
                   ...
                   $existe=mysqli_num_rows(mysqli_query($conexao, "select idaluno from matriculaseconfirmacoes where idaluno='$idaluno' and idturma='$idturma'"));
                         ...
                         if($existe==0){
                           ...
                           $salvar=mysqli_query($conexao,"INSERT INTO `matriculaseconfirmacoes` (`idmatriculaeconfirmacao`, `idaluno`, `idanolectivo`, `idturma`, `tipo`, `preco`, `desconto`, `valorpago`, `turma`, `sala`, `curso`, `periodo`, `classe`, data,obs, tipodealuno, descontoparapropinas) VALUES (NULL, '$idaluno', '$idanolectivo', '$idturma', 'Confirmação', '$preco', '$desconto', '$valorpago', '$turma', '$sala', '$curso', '$periodo', '$classe', STR_TO_DATE('$datadareconfirmacao', '%d/%m/%Y'), '$obsreconfirmacao', '$tipodealuno', '$descontoparapropinas')");
                     ...
                     $datadehoje=Date("Y-m-d");
                          ...
                          if($salvar){
                               ...
                               $mudar_a_reconfirmacaopassada=mysqli_query($conexao,"UPDATE `matriculaseconfirmacoes` SET `reconfirmou` = '1' WHERE `matriculaseconfirmacoes`.`idmatriculaeconfirmacao` = '$idmatriculaeconfirmacao'");
                                 ...
                                 $descricao="Registro de Confirmação";
                                $salvar_financas=mysqli_query($conexao,"INSERT INTO `entradas` (`identrada`, `idfuncionario`, `descricao`, `tipo`, `idtipo`, `valor`, `divida`, `idaluno`, `idturma`, `datadaentrada`, formadepagamento, idanolectivo) VALUES (NULL, '$idlogado', '$descricao', 'Confirmação', '$idmatriculaeconfirmacao', '$valorpago', '$divida', '$idaluno', '$idturma', STR_TO_DATE('$datadareconfirmacao', '%d/%m/%Y'), '$formadepagamento', '$idanolectivo')");

                                      $identrada=mysqli_fetch_array(mysqli_query($conexao, "SELECT identrada from  entradas where idaluno='$idaluno' and tipo='Confirmação' order by identrada desc limit 1"))[0]; 
                                          ...
                                          if($salvar_financas){  
                                            ...
                                            $erros[]="Ocorreu um erro ao fazer a Confirmação do aluno | No Registro de financas";
                                     ...
                                     $erros[]="Ocorreu um erro ao fazer a Confirmação do aluno";
                                }
                        }else{

                                     $erros[]="Já existe uma Confirmação desse aluno nessa turma";
                                }
      ...
      <?php } ?>
          ...
          <div class="row">
            ...
            <div class="col-lg-7">
              ...
              <div class="card shadow mb-4">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary">Confirmação</h6>
                </div>
                <div class="card-body">
            ...
            <script>
             ...
             turma.addEventListener("change", function(){
                    ...
                    var idturma=this.value;
                      ...
                      if(idturma=='0'){
                         ...
                         $.ajax({
                          url:"cadastro/pesquisarturmareconfirmacao.php",
                          method:"POST",
                          data:{idturma},
             ...
             anolectivo.addEventListener("change", function(){
                    ...
                    var idanolectivo=this.value;
                     $.ajax({
                          url:"cadastro/pesquisaranolectivoreconfirmacao.php",
                          method:"POST",
                          data:{idanolectivo},
                          success:function(data){
