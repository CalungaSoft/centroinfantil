...
if (isset($_POST['cadastrar'])) {
  ...
  if (!empty(trim($_POST['titulo']))) {
    ...
    $existe = mysqli_num_rows(mysqli_query($conexao, "select id from tiposdeavalicoes where titulo='$titulo' and idcategoria='$idcategoria'"));

    if ($existe == 0) {

      $salvar = mysqli_query($conexao, "INSERT INTO `tiposdeavalicoes` (titulo,idcategoria) VALUES ('$titulo','$idcategoria')");
 

      if ($salvar) {
 

        $acerto[] = "Avaliação $titulo foi Cadastrado com sucesso";
      } else {
      ...
      $erros[] = "Essa Avaliação já existe";
    }
  } else {
...
<div class="container-fluid">
  ...
  <!-- Page Heading -->
  <h1 class="h3 mb-2 text-gray-800">Lista de avaliações</h1>
  <p class="mb-4">A seguir vai a lista de avaliações</p>
  ...
  <button id="myBtn" class="btn btn-success"> <i class="fas fa-fw fa-plus"></i> Acrescentar avaliação</button>
  ...
  <div id="myModal" class="modal">
    <div class="modal-content">
      ...
      <form class="user" method="post" action="">
        <h3>Cadastrando uma nova  Avaliação</h3> <br>
        <div class="form-group">
          ...
          ?>
          <input type="text" name="titulo" autocomplete="off" list="datalist2" class="form-control" placeholder="Avaliação" required="">
