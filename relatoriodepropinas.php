...
$mesespecifico=isset($_GET['mesespecifico'])?$_GET['mesespecifico']:"";
 ...
 if(isset($_GET['mesespecifico'])){
      ...
      $total_cobrado_semmulta=mysqli_fetch_array(mysqli_query($conexao,"SELECT sum(preco) from propinas where idanolectivo='$idanolectivo' and mespago='$mesespecifico'"))[0];

      $total_em_multa=mysqli_fetch_array(mysqli_query($conexao,"SELECT sum(multa) from propinas where idanolectivo='$idanolectivo' and mespago='$mesespecifico'"))[0];

      $total_cobrado_commulta=$total_em_multa+$total_cobrado_semmulta;

      $total_em_desconto=mysqli_fetch_array(mysqli_query($conexao,"SELECT sum(desconto) from propinas where idanolectivo='$idanolectivo' and mespago='$mesespecifico'"))[0];
      $total_em_valorpago=mysqli_fetch_array(mysqli_query($conexao,"SELECT sum(valorpago) from propinas where idanolectivo='$idanolectivo' and mespago='$mesespecifico'"))[0];
      $total_em_divida=mysqli_fetch_array(mysqli_query($conexao,"SELECT sum(preco+multa-desconto-valorpago) from propinas where idanolectivo='$idanolectivo' and mespago='$mesespecifico'"))[0];

 }else{


    $total_cobrado_semmulta=mysqli_fetch_array(mysqli_query($conexao,"SELECT sum(preco) from propinas where idanolectivo='$idanolectivo'"))[0];

      $total_em_multa=mysqli_fetch_array(mysqli_query($conexao,"SELECT sum(multa) from propinas where idanolectivo='$idanolectivo'"))[0];

      $total_cobrado_commulta=$total_em_multa+$total_cobrado_semmulta;

      $total_em_desconto=mysqli_fetch_array(mysqli_query($conexao,"SELECT sum(desconto) from propinas where idanolectivo='$idanolectivo'"))[0];
      $total_em_valorpago=mysqli_fetch_array(mysqli_query($conexao,"SELECT sum(valorpago) from propinas where idanolectivo='$idanolectivo'"))[0];
      $total_em_divida=mysqli_fetch_array(mysqli_query($conexao,"SELECT sum(preco+multa-desconto-valorpago) from propinas where idanolectivo='$idanolectivo'"))[0];
...
include("cabecalho.php") ; ?>
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-2 text-gray-800">Relatório de Propinas do ano lectivo <a href="anolectivo.php?idanolectivo=<?php echo "$idanolectivo"; ?>"><?php echo "$nomedoanolectivo"; ?></a> <?php if(isset($_GET['mesespecifico'])){ echo "($nomedomes)"; }?> </h1>
                         ...
                         <div class="col-sm-12"> 
                                <span>Mês Específico</span>
                                  <select name="mesespecifico" required  class="form-control"> 
                                    <?php
                                         $lista=mysqli_query($conexao,"SELECT distinct(mespago), YEAR(propinas.mespago) as ano, MONTH(propinas.mespago) as mes from propinas where idanolectivo='$idanolectivo' order by mespago desc");
  ...
  <button id="myBtnreclamacoes" class="btn btn-primary" >Escolher outro ano lectivo</button>   
    ...
    <a href="pdf/pdfalunoscompropinaspagas.php?idanolectivo=<?php echo $idanolectivo; ?>" class="d-sm-inline-block btn btn-sm btn-info" ><i class="fas fa-fw fa-print"></i> Imprimir Em forma de tabela</a>
                ...
                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                  ...
                  <?php
                         ...
                         if(isset($_GET['mesespecifico'])){
                             ...
                             $lista=mysqli_query($conexao, "SELECT alunos.nomecompleto,  YEAR(propinas.mespago) as ano, MONTH(propinas.mespago) as mes, matriculaseconfirmacoes.*, propinas.* from matriculaseconfirmacoes, propinas, alunos where matriculaseconfirmacoes.idaluno=alunos.idaluno and propinas.idanolectivo='$idanolectivo' and propinas.idmatriculaeconfirmacao=matriculaseconfirmacoes.idmatriculaeconfirmacao and mespago='$mesespecifico'");
 
                               }else{

                              $lista=mysqli_query($conexao, "SELECT alunos.nomecompleto,  YEAR(propinas.mespago) as ano, MONTH(propinas.mespago) as mes, matriculaseconfirmacoes.*, propinas.* from matriculaseconfirmacoes, propinas, alunos where matriculaseconfirmacoes.idaluno=alunos.idaluno and propinas.idanolectivo='$idanolectivo' and propinas.idmatriculaeconfirmacao=matriculaseconfirmacoes.idmatriculaeconfirmacao");
