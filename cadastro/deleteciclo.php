...
if(isset($_POST["idciclo"])){
    ...
    $erro=0;
    //Entradas
    $registros_alunos_cadastrados=mysqli_num_rows(mysqli_query($conexao, "SELECT   turmas.idturma FROM matriculaseconfirmacoes, turmas where turmas.idciclo='$idciclo' and matriculaseconfirmacoes.idturma=turmas.idturma ")); 
   ...
   if ($erro==0) {
    ...
    $eliminandoidciclo=mysqli_query($conexao," DELETE FROM ciclos where idciclo='$idciclo'");
      ...
      if($eliminandoidciclo){
            echo '<div class="alert alert-success"> Nível Eliminado com Sucesso <a href="index.php">Clique aqui para ir a página inicial!</a> </div>';
       }
       else{
        echo '<div class="alert alert-danger">Ocorreu um erro ao eliminar o Nível!</div>';
