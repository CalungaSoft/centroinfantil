   ...
   $id=mysqli_escape_string($conexao, $_POST['id']);
        ...
        $dados=mysqli_fetch_array(mysqli_query($conexao," SELECT * FROM entradas where identrada='$id'"));
         ...
         $outro_id=mysqli_fetch_array(mysqli_query($conexao," SELECT identrada FROM entradas where idtipo='$idtipo' and tipo='$tipo' and identrada!='$id' limit 1"))[0];
           ...
           $antigo="$descricaonaentrada | Valor: $valorantigoh KZ <a href=entradapropinadotransporte.php?identrada=$outro_id>Clique para ver</a>";
           $novo="Eliminado";
           ...
           $guardar=mysqli_query($conexao,"INSERT INTO `historico` (`idhistorico`, `idfuncionario`, `descricao`, `antigo`, `novo`, `data`) VALUES (NULL, '$idfuncionario', 'Eliminação', '$antigo', '$novo', CURRENT_TIMESTAMP)");
            ...
            $actualizandopropinas=mysqli_query($conexao,"UPDATE `propinasdotransporte` SET `valorpago` =`valorpago`-'$valorantigo'  WHERE `propinasdotransporte`.`idpropinadotransporte` = '$idtipo'");
             ...
             $preco=mysqli_fetch_array(mysqli_query($conexao," SELECT sum(preco+multa-desconto) as preco FROM propinasdotransporte where idpropinadotransporte='$idtipo' "))['preco'];
        ...
        if($insirindo_dividas){
            echo '<div class="alert alert-success"> Registro Eliminado com Sucesso | Os cálcos com os novos valores serão feitos após a actualização da página, <a href="entradapropinadotransporte.php?identrada='.$outro_id.'">Clique aqui para actualizar a página!</a> </div>';
       }
       else{
