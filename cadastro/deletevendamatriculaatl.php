   ...
   $id=mysqli_escape_string($conexao, $_POST['id']);
        ...
        $dados=mysqli_fetch_array(mysqli_query($conexao," SELECT * FROM entradas where identrada='$id'"));
          ...
          $primeiroid=mysqli_fetch_array(mysqli_query($conexao," SELECT identrada FROM entradas where idtipo='$idtipo' and tipo='$tipo' order by identrada asc limit 1"))[0];
           ...
           $antigo="Todos os dados de($descricaonaentrada) | Valor: $valorantigoh KZ | Por Consolidar $dividaantigo KZ | <a href=entradadamatriculadoatl.php?identrada=$primeiroid>Clique para ver</a>";
           $novo="Eliminado";
           ...
           $guardar=mysqli_query($conexao,"INSERT INTO `historico` (`idhistorico`, `idfuncionario`, `descricao`, `antigo`, `novo`, `data`) VALUES (NULL, '$idfuncionario', 'Eliminação', '$antigo', '$novo', CURRENT_TIMESTAMP)");
            ...
            $actualizandopagamentos=mysqli_query($conexao,"UPDATE `matriculaatl` SET `valorpago` ='0'  WHERE `matriculaatl`.`idmatriculaatl` = '$idtipo'");
 
             $zerando_dividas=mysqli_query($conexao,"UPDATE `entradas` SET  `divida` ='0', `valor` ='0' WHERE tipo='$tipo' and idtipo='$idtipo'");

             $divida_nova=mysqli_fetch_array(mysqli_query($conexao," SELECT sum(preco-desconto) as preco FROM matriculaatl where idmatriculaatl='$idtipo' "))['preco'];
        ...
        if(mysqli_query($conexao, "Delete from entradas where idtipo='$idtipo' and tipo='$tipo' and identrada!='$primeiroid'")){
            echo '<div class="alert alert-success"> Registro Eliminado com Sucesso | Os cálcos com os novos valores serão feitos após a actualização da página, <a href="entradadamatriculadoatl.php?identrada='.$primeiroid.'">Clique aqui para actualizar a página!</a> </div>';
       }
       else{
