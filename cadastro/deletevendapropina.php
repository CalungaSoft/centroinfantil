...
if(isset($_POST["id"])){
   ...
   $id=mysqli_escape_string($conexao, $_POST['id']);
        ...
        $dadosdapropinas=mysqli_fetch_array(mysqli_query($conexao," SELECT YEAR(mespago) as ano, MONTH(mespago) as mes, propinas.* FROM propinas where idpropina='$idtipo'"));
   
            $mes=$dadosdapropinas["mes"];
            $ano=$dadosdapropinas["ano"];

            $idpropina=$dadosdapropinas["idpropina"];
            $idaluno=$dadosdapropinas["idaluno"];

            $idmatriculaeconfirmacao=$dadosdapropinas["idmatriculaeconfirmacao"];

            $nomedoaluno=mysqli_fetch_array(mysqli_query($conexao, "select nomecompleto from alunos where idaluno='$idaluno'"))[0];
        
        $pagamentoanterior=mysqli_num_rows(mysqli_query($conexao," SELECT * FROM propinas where idmatriculaeconfirmacao='$idmatriculaeconfirmacao'"));
    
        if($pagamentoanterior==1){

            $Actualizando=mysqli_query($conexao,"UPDATE `matriculaseconfirmacoes` SET `ultimomespago` = '0000-00-00' WHERE `matriculaseconfirmacoes`.`idmatriculaeconfirmacao` = '$idmatriculaeconfirmacao'");

              if ($Actualizando) {
                
                $elimindo=mysqli_query($conexao, "Delete from propinas where idpropina='$idtipo'");

                    if ($elimindo) {
                         
                        $antigo="Eliminado o pagamento de propinas do aluno  <a href=aluno.php?idaluno=$idaluno>$nomedoaluno </a> do mês de 0$mes/$ano";
                        $novo="Eliminado";
                            ...
                            if($guardar){
                                ...
                                $elimindodasentradas=mysqli_query($conexao, "Delete from entradas where idtipo='$idtipo' and tipo='$tipo' and idaluno='$idaluno'");
                                    ...
                                    if ($elimindodasentradas) {
                                        echo '<div class="alert alert-success"> Todos  os registros relacionados com essa propinas foram elimanadas com sucesso!  </div>';
                                    }else{
                            ...
                            if($guardar){ 
                                ...
                                $elimindodasentradas=mysqli_query($conexao, "Delete from entradas where idtipo='$idtipo' and tipo='$tipo' and idaluno='$idaluno'");
                                    ...
                                    if ($elimindodasentradas){
                                        ...
                                        $maiorano=mysqli_fetch_array(mysqli_query($conexao," SELECT YEAR(mespago) FROM propinas where idmatriculaeconfirmacao='$idmatriculaeconfirmacao' order by YEAR(mespago) desc limit 1"))[0];
                                        $maiormes=mysqli_fetch_array(mysqli_query($conexao," SELECT MONTH(mespago) FROM propinas where idmatriculaeconfirmacao='$idmatriculaeconfirmacao'  and YEAR(mespago)='$maiorano' order by MONTH(mespago) desc limit 1"))[0];
                                        ...
                                        $Actualizando=mysqli_query($conexao,"UPDATE `matriculaseconfirmacoes` SET `ultimomespago` = STR_TO_DATE('$ultimopagamentodoaluno', '%d/%m/%Y') WHERE `matriculaseconfirmacoes`.`idmatriculaeconfirmacao` = '$idmatriculaeconfirmacao'");
      
                                        if ($Actualizando) {
                                            
                                            echo '<div class="alert alert-success">Todos  os registros relacionados com essa propinas foram elimanadas com sucesso! <a href=index.php> clique aqui para actualizar a página</a></div>';
