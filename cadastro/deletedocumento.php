   ...
   $id=mysqli_escape_string($conexao, $_POST['id']);
        ...
        $dados=mysqli_fetch_array(mysqli_query($conexao," SELECT * FROM entradas where identrada='$id'"));
         ...
         $outro_id=mysqli_fetch_array(mysqli_query($conexao," SELECT identrada FROM entradas where idtipo='$idtipo' and tipo='$tipo' and identrada!='$id' limit 1"))[0];
           ...
           $antigo="$descricaonaentrada | Valor: $valorantigoh KZ <a href=detalhestratardocumentos.php?identrada=$outro_id>Clique para ver</a>";
           $novo="Eliminado";
           ...
           $guardar=mysqli_query($conexao,"INSERT INTO `historico` (`idhistorico`, `idfuncionario`, `descricao`, `antigo`, `novo`, `data`) VALUES (NULL, '$idfuncionario', 'Eliminação', '$antigo', '$novo', CURRENT_TIMESTAMP)");
            ...
            $actualizandopagamentos=mysqli_query($conexao,"UPDATE `documentostratados` SET `valorpago` =`valorpago`-'$valorantigo'  WHERE `documentostratados`.`iddocumentotratado` = '$idtipo'");

             $deletandodaentrada=mysqli_query($conexao, "Delete from entradas where identrada='$id'");
        ...
        if($insirindo_dividas){
            echo '<div class="alert alert-success"> Registro Eliminado com Sucesso | Os cálcos com os novos valores serão feitos após a actualização da página, <a href="detalhestratardocumentos.php?identrada='.$outro_id.'">Clique aqui para actualizar a página!</a> </div>';
       }
       else{
