   ...
   $id=mysqli_escape_string($conexao, $_POST['id']);
        ...
        $dados=mysqli_fetch_array(mysqli_query($conexao," SELECT * FROM entradas where identrada='$id'"));
         ...
         $outro_id=mysqli_fetch_array(mysqli_query($conexao," SELECT identrada FROM entradas where idtipo='$idtipo' and tipo='$tipo' and identrada!='$id' limit 1"))[0];
           ...
           $antigo="$descricaonaentrada | Valor: $valorantigoh KZ <a href=entradapropinadoatl.php?identrada=$outro_id>Clique para ver</a>";
           $novo="Eliminado";
           ...
           $guardar=mysqli_query($conexao,"INSERT INTO `historico` (`idhistorico`, `idfuncionario`, `descricao`, `antigo`, `novo`, `data`) VALUES (NULL, '$idfuncionario', 'Eliminação', '$antigo', '$novo', CURRENT_TIMESTAMP)");
            ...
            $actualizandopropinas=mysqli_query($conexao,"UPDATE `propinasdoatl` SET `valorpago` =`valorpago`-'$valorantigo'  WHERE `propinasdoatl`.`idpropinadoatl` = '$idtipo'");
             ...
             $preco=mysqli_fetch_array(mysqli_query($conexao," SELECT sum(preco+multa-desconto) as preco FROM propinasdoatl where idpropinadoatl='$idtipo' "))['preco'];
        ...
        if($insirindo_dividas){
            echo '<div class="alert alert-success"> Registro Eliminado com Sucesso | Os cálcos com os novos valores serão feitos após a actualização da página, <a href="entradapropinadoatl.php?identrada='.$outro_id.'">Clique aqui para actualizar a página!</a> </div>';
       }
       else{
