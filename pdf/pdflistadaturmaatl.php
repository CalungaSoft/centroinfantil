   ...
   $idatl=isset($_GET['idatl'])?$_GET['idatl']:"0";
   
       $dados_do_atl=mysqli_fetch_array(mysqli_query($conexao,"SELECT * from atl where idatl='$idatl'"));
   
       $idanolectivo=$dados_do_atl["idanolectivo"];
    
       $atl=$dados_do_atl["titulo"];
       $descricao=$dados_do_atl["descricao"];
   ...
   $dia=date('d');  
           ...
           <div>
               <div>
                   ...
                   <hr><hr>
                       ...
                       <span style="font-size: 15px; margin-left:30px"> LISTA NOMINAL DOS ALUNOS DO ATL  <strong>  '.$atl.' | '.$anolectivo.' </strong> </span>
     ...
     <tbody> 
          "; 
              ...
              $lista=mysqli_query($conexao, "select TIMESTAMPDIFF(YEAR,datadenascimento,CURDATE()) as idade, alunos.nomecompleto, alunos.numerodeprocesso, alunos.sexo,  matriculaatl.* from matriculaatl, alunos where matriculaatl.idaluno=alunos.idaluno and matriculaatl.idatl='$idatl' order by alunos.nomecompleto "); 
