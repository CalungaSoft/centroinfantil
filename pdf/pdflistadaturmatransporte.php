   ...
   $idtransporte=isset($_GET['idtransporte'])?$_GET['idtransporte']:"0";
   
       $dados_do_transporte=mysqli_fetch_array(mysqli_query($conexao,"SELECT * from transportes where idtransporte='$idtransporte'"));
   
       $idanolectivo=$dados_do_transporte["idanolectivo"];
    
       $transporte=$dados_do_transporte["titulo"];
       $descricao=$dados_do_transporte["descricao"];
       $pessoal=$dados_do_transporte["pessoal"];
   ...
   $dia=date('d');  
           ...
           <div>
               <div>
                   ...
                   <hr><hr>
                       ...
                       <span style="font-size: 15px; margin-left:30px"> LISTA NOMINAL DOS ALUNOS DO TRANSPORTE  <strong>  '.$transporte.' | '.$anolectivo.' </strong> </span>
                       <br><span style="font-size: 12px; margin-left:30px">  <strong> Descrição </strong> :  '.$descricao.' | <strong> Motorista e Auxiliares </strong> :  '.$pessoal.'</span>
               <br> <br>   ';
     ...
     <tbody> 
          "; 
              ...
              $lista=mysqli_query($conexao, "select TIMESTAMPDIFF(YEAR,datadenascimento,CURDATE()) as idade, alunos.nomecompleto, alunos.numerodeprocesso, alunos.sexo,  matriculatransporte.* from matriculatransporte, alunos where matriculatransporte.idaluno=alunos.idaluno and matriculatransporte.idtransporte='$idtransporte' order by alunos.nomecompleto "); 
   ...
   ___________________________ <br>
       ".$dadosdainstituicao['nomedodireitor']."
          ...
          // $gerador->setPaper('A4', 'landscape');
           ...
           $gerador->stream(
               "listaDosalunosDotransporteCalungaSoft.pdf",
               array(
                   "attachment" => true
               )
           );
