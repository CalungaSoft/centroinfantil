...
$dia=date('d');  
    ...
    $dadosdainstituicao=mysqli_fetch_array(mysqli_query($conexao,"select * from dadosdaempresa"));
        ...
        <div>
            <div>
                ...
                <hr><hr>
                    ...
                    <span style="font-size: 15px; margin-left:30px"> |Presença dos Professores: '.$mesdevenda.' / '.$anodevenda.'</span>
            <br> <br> <br> 
        <table style="border:0px solid; border-spacing:0px; margin-top:-20px;  padding:20px" width="90%" align=center>
                  
                  <thead>   
                  <tr>
                      <th  width="auto" style="border: 1px solid; border-spacing:0px">Funcionario</th> 
                  ...
                  while($exibir = $listadefuncionários_auxiliares->fetch_array()){ 
                    ...
                    foreach ($vetor_de_professor as $key => $value) {
                          ...
                          $dados_do_funcionario=mysqli_fetch_array(mysqli_query($conexao,"SELECT nomedofuncionario, salario FROM  funcionarios where idfuncionario='$idfuncionario'"));
                           ...
                           $numero_de_dias=mysqli_num_rows(mysqli_query($conexao,"SELECT DISTINCT(DAY(diadapresenca)) FROM  presencaprofessores, disciplinas where  presencaprofessores.iddisciplina=disciplinas.iddisciplina and disciplinas.idanolectivo='$idanolectivo'  and presencaprofessores.idprofessor='$idfuncionario' and YEAR(diadapresenca)='$anodevenda' and MONTH(diadapresenca)='$mesdevenda'"));



                           $total_de_tempos=mysqli_fetch_array(mysqli_query($conexao,"SELECT sum(totaldetempos) FROM  presencaprofessores, disciplinas where  presencaprofessores.iddisciplina=disciplinas.iddisciplina and disciplinas.idanolectivo='$idanolectivo' and presencaprofessores.idprofessor='$idfuncionario' and YEAR(diadapresenca)='$anodevenda' and MONTH(diadapresenca)='$mesdevenda'"))[0];



                           $salario_acumulado=mysqli_fetch_array(mysqli_query($conexao,"SELECT sum(totaldetempos*salarioportempo) FROM  presencaprofessores, disciplinas where  presencaprofessores.iddisciplina=disciplinas.iddisciplina and disciplinas.idanolectivo='$idanolectivo' and presencaprofessores.idprofessor='$idfuncionario' and YEAR(diadapresenca)='$anodevenda' and MONTH(diadapresenca)='$mesdevenda'"))[0];
                         ...
                         <tr>  
                              <td width='auto' style='border: 1px solid; border-spacing:0px'>".$dados_do_funcionario["nomedofuncionario"]."</td>
                         ...
                         $total_geral_dias=mysqli_num_rows(mysqli_query($conexao,"SELECT DISTINCT(DAY(diadapresenca)) FROM  presencaprofessores, disciplinas where  presencaprofessores.iddisciplina=disciplinas.iddisciplina and disciplinas.idanolectivo='$idanolectivo'  and YEAR(diadapresenca)='$anodevenda' and MONTH(diadapresenca)='$mesdevenda'"));
                        ...
                        $htm.="
                         <tfoot>
                        <tr>
                            <th width='auto' style='border: 1px solid; border-spacing:0px'><strong>Total</strong></th>
                            <th width='auto' style='border: 1px solid; border-spacing:0px'> </th> 
                            <th width='auto' style='border: 1px solid; border-spacing:0px'>".$total_geral_salariobase." </th> 
                            <th width='auto' style='border: 1px solid; border-spacing:0px'>".$total_geral_dias."</th> 
                            <th width='auto' style='border: 1px solid; border-spacing:0px'>".$total_geral_detempos."</th> 
                            <th width='auto' style='border: 1px solid; border-spacing:0px'>".$total_geral_salarioacumulado."| (".$percentagem." %) </th>  
                        </tr>  
                    </tfoot>
                    </tbody>
             </table> 
     ...
     $gerador->load_html($htm); 
        ...
        $gerador->stream(
            "FolhaDeSalário dos professores - ".$anodevenda." - ".$mesdevenda." - CalungaSoft.pdf",
            array(
                "attachment" => true
            )
        );
