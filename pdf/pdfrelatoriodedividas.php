    ...
    $dadosdainstituicao=mysqli_fetch_array(mysqli_query($conexao,"select * from dadosdaempresa"));
        ...
        <div>
            <div>
                ...
                <hr><hr>
                    ...
                    <span style="font-size: 15px; margin-left:30px"> |Relatório De Dívida: '.$mesdevenda.' / '.$anodevenda.'</span>
            <br> <br> <br> 
            |Entradas<br>
                ...
                <tbody> 
                     ';  
                       ...
                       if(!isset($_GET['mesdevenda'])){
                          $registrosdeentrada=mysqli_query($conexao, "select entradas.*, funcionarios.nomedofuncionario from entradas, funcionarios where funcionarios.idfuncionario=entradas.idfuncionario and divida!=0 order by entradas.identrada desc"); 
                        }
                       else { 
                        $registrosdeentrada=mysqli_query($conexao,"select  entradas.*, funcionarios.nomedofuncionario from entradas, funcionarios  where '$anodevenda'=YEAR(entradas.datadaentrada) AND '$mesdevenda'=MONTH(entradas.datadaentrada) and funcionarios.idfuncionario=entradas.idfuncionario and divida!=0 order by entradas.identrada desc"); 
                      }
      ...
      <tbody> 
           ';  
              ...
              $registrosdeentrada=mysqli_query($conexao, "select distinct(tipo) from entradas where '$anodevenda'=YEAR(datadaentrada) AND '$mesdevenda'=MONTH(datadaentrada) and divida!='0' order by tipo asc"); 
...
<br><br>";
  ...
  $htm.="
        ...
        $gerador->stream(
            "relatoriodedividasCalungaSoft.pdf",
            array(
                "attachment" => true
            )
        );
